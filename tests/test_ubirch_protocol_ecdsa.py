...
NRM_SIGNED_UPP = (
    bytes.fromhex(
        "9522c4106eac4d0b16e645088c4622e7451ea5a100c4207bc6b089b5c33f1e0d319795851c257f11cdb7ea33052acd17f0ae2c1bdf8814c44075e08e6390e4160cd9703a322f6bd697d2aa1917b3032c039ae15ce03f1fb9f2510d130196514c661934cd63ef55a34e1d6e8f8c6f50f25479032c6ac397af3e"
        # upp
    ),
    bytes.fromhex("7bc6b089b5c33f1e0d319795851c257f11cdb7ea33052acd17f0ae2c1bdf8814")  # payload
)
NRM_SIGNED_UPP_FROM_HASH = bytes.fromhex(
    ""
)

# keys defined multiple times in case tests should be modified at some point
# normal chained upps
NRM_CHAINED_UPPS_UUID = NRM_SIGNED_UPPS_UUID
NRM_CHAINED_UPPS_PRIV = NRM_SIGNED_UPPS_PRIV
NRM_CHAINED_UPPS_PUB = NRM_SIGNED_UPPS_PUB
# tuple of upp/payload
NRM_CHAINED_UPPS = [
    (bytes.fromhex(
...
SIM_SIGNED_UPPS = [
    (bytes.fromhex(
        "9522c4100710423318924020904200002f6d169d00c420c86b9e8dc30aba9ae823b9735eeac26a9f8139f7528ea9599c89c5d8be52a093c44039865534ed13ed4152b99d5387f9b6b2faba800f811db73066eab48989fcc1c7a0df8dec55d5599f858dd78f9e77baf4dbc696663417e08316f2d9f660004dbe"
    ),  # upp
     bytes.fromhex("c86b9e8dc30aba9ae823b9735eeac26a9f8139f7528ea9599c89c5d8be52a093")  # payload
    ),
    (bytes.fromhex(
        "9522c4100710423318924020904200002f6d169d00c42020977c685c1d230b0e58b08846434924ad345694afbe2b3bac8fd3885ccc3a51c440c5baf5853da4b15b6c861bd04c6cb721e3b62ce37e17fbcb04e92ed483fee083bd68d9e99fec16cf332264d20c01be86f1aba9a20ecea10d7afd1992b0633bf2"
    ),  # upp
     bytes.fromhex("20977c685c1d230b0e58b08846434924ad345694afbe2b3bac8fd3885ccc3a51")  # payload
    ),
    (bytes.fromhex(
        "9522c4100710423318924020904200002f6d169d00c42057b61a0b0d73ec7be87711f5d46273521e03197b6ce1e5dea45c7865b25456d7c440b1217b28b38494d037af9c51e59c363bb134e131690325a3a58fdc4cf32e5f7517fd63fe7e06ab8ed8207ce178ae70fc8dec5c1647baa29bf21230309e01ec41",
    ),  # upp
     bytes.fromhex("57b61a0b0d73ec7be87711f5d46273521e03197b6ce1e5dea45c7865b25456d7")  # payload
    ),
    (bytes.fromhex(
        "9522c4100710423318924020904200002f6d169d00c4205bb3fbd13b9e2d4c5e7499c02320e8db74edec1fd66cb1dd06ab3e3b72bccdaac4406601058cb6dc81254fdc65363c89de673220be111a3c015e0f539a4d0500ce0a5ce7f67c1b7182930f8fbed37614d25be004a5a4885f5e75435f88ea51de237f"
    ),  # upp
...
# keys defined multiple times in case tests should be modified at some point
# sim chained upps
SIM_CHAINED_UPPS_UUID = SIM_SIGNED_UPPS_UUID
SIM_CHAINED_UPPS_PUB = SIM_SIGNED_UPPS_PUB
# tuples of upp/payload
SIM_CHAINED_UPPS = [
    (bytes.fromhex(
        ...
        bytes.fromhex("d2233cd27a9ad0493433e144344add27c524b09793a8b65dea4d4e72e9335dde")
    ),
    (bytes.fromhex(
        ...
        bytes.fromhex("0d5be1990e1d880acc08c4e88b5324947cdb5c3edf1a5470adbd217b69498966")
    ),
    (bytes.fromhex(
        ...
        bytes.fromhex("50d8537ff48d337d4cddce62cad2e436071616571100ee41ece80d1b7af50045")
    ),
    (bytes.fromhex(
        ...
        bytes.fromhex("5e5e21637343130643d1971cefd801bfee7688bbab221ae9e444393fc5d5c870")
    )
]

# helper for keyloading ecdsa
...
class TestUbirchProtocolECDSA(unittest.TestCase):
    ...
    # p.verify_signature() and p.unpack_upp() are already tested in `test_ubirch_protocol_ed25519.py` therefore safe to use here
    def test_create_verify_unpack_signed_message(self):
        p = Protocol()
        ...
        message = p.message_signed(NRM_SIGNED_UPPS_UUID, PAYLOAD_TYPE, PAYLOAD) # create chained upp

        self.assertEqual(p.verify_signature(NRM_SIGNED_UPPS_UUID, message), True) # check whether verifying it works
        unpacked = p.unpack_upp(message) # unpack the UPP to check components

        self.assertEqual(unpacked[2], PAYLOAD_TYPE) # compare the payload type and payload
        self.assertEqual(unpacked[3], PAYLOAD)

    def test_create_verify_unpack_signed_message_with_hash(self):
        p = Protocol()
        p.sk = _sk_bytes_to_sk(NRM_SIGNED_UPPS_PRIV)
        p.vk = _vk_bytes_to_vk(NRM_SIGNED_UPPS_PUB)

        PAYLOAD_TYPE = 0xEF
        PAYLOAD = hashlib.sha512(b'1').digest() # Here a hash is used instead of the payload itself
    ...
    def test_create_verify_unpack_chained_messages(self):
        p = Protocol()
        ...
        for i in range(0, 3):
            ...
            self.assertEqual(p.verify_signature(NRM_CHAINED_UPPS_UUID, message), True) # check whether verifying it works
            unpacked = p.unpack_upp(message) # unpack the UPP to check components

            self.assertEqual(unpacked[2], last_signature) # compare the prevsig
            last_signature = unpacked[5] # set the prevsig to the current sig

            self.assertEqual(unpacked[3], PAYLOAD_TYPE) # compare the payload type and payload
            self.assertEqual(unpacked[4], i + 1)

    def test_create_verify_unpack_chained_message_with_hash(self):
        p = Protocol()
        p.sk = _sk_bytes_to_sk(NRM_CHAINED_UPPS_PRIV)
        p.vk = _vk_bytes_to_vk(NRM_CHAINED_UPPS_PUB)

        PAYLOAD_TYPE = 0xEE
        PAYLOAD = hashlib.sha512(b'1').digest()
        message = p.message_chained(NRM_CHAINED_UPPS_UUID, PAYLOAD_TYPE, PAYLOAD)

        self.assertEqual(p.verify_signature(NRM_CHAINED_UPPS_UUID, message), True) # check whether verifying it works
        unpacked = p.unpack_upp(message) # unpack the UPP to check components

        self.assertEqual(unpacked[3], PAYLOAD_TYPE) # compare the payload type and payload
        self.assertEqual(unpacked[4], PAYLOAD)
    ...
    def test_verify_signed_message(self):
        ...
        self.assertEqual(p.verify_signature(NRM_SIGNED_UPPS_UUID, NRM_SIGNED_UPP[0]), True) # verify the upp
        unpacked = p.unpack_upp(NRM_SIGNED_UPP[0]) # unpack the upp

        self.assertEqual(SIGNED, unpacked[0]) # check if the type is right
        self.assertEqual(NRM_SIGNED_UPPS_UUID.bytes, unpacked[1]) # check the uuid
        self.assertEqual(NRM_SIGNED_UPP[1], unpacked[3]) # check the payload
    ...
    def test_verify_chained_messages(self):
        ...
        for upp in NRM_CHAINED_UPPS:
            self.assertEqual(p.verfiy_signature(NRM_CHAINED_UPPS_UUID, upp[0]), True) # verify the upp
            unpacked = p.unpack_upp(upp[0]) # unpack the upp

            self.assertEqual(NRM_CHAINED_UPPS_UUID.bytes, unpacked[1]) # check the uuid
            self.assertEqual(upp[1], unpacked[4]) # check the payload

            if prev != None:
                self.assertEqual(unpacked[2], prev) # check the prevsig

            prev = unpacked[-1] # set the prevsig

class TestEcdsaProtocolSecurity(unittest.TestCase):
    # Ecdsa signature has to change with every time sk.sign() is called
    def test_signature_changes(self):
        ...
        seen_signatures = {}
        for _ in range(1000):
            signed_msg = p._sign("UUID", b'1')
            signed_msg_hex = signed_msg.hex()

            if signed_msg_hex in seen_signatures:
                print("duplicate signature!")
                break
            else:
                seen_signatures[signed_msg_hex] = True

    # TODO Applicable? Use new protocol (context?) for each test run -> like in protocol.go
    def test_signature_changes_new_protocol(self):
        seen_signatures = {}
...
class TestUbirchProtocolSIM(unittest.TestCase):
    ...
    def test_verify_signed_message_sim_v2(self):
        ...
        for upp in SIM_SIGNED_UPPS:
            self.assertEqual(p.verfiy_signature(SIM_SIGNED_UPPS_UUID, upp[0]), True) # verify the upp
            unpacked = p.unpack_upp(upp[0]) # unpack the upp

            self.assertEqual(SIM_SIGNED_UPPS_UUID.bytes, unpacked[1]) # check the uuid
            self.assertEqual(upp[1], unpacked[3]) # check the payload
    ...
    def test_verify_chained_message_sim_v2(self):
        ...
        for upp in SIM_CHAINED_UPPS:
            self.assertEqual(p.verfiy_signature(SIM_CHAINED_UPPS_UUID, upp[0]), True) # verify the upp
            unpacked = p.unpack_upp(upp[0]) # unpack the upp

            self.assertEqual(SIM_CHAINED_UPPS_UUID.bytes, unpacked[1]) # check the uuid
            self.assertEqual(upp[1], unpacked[4]) # check the payload

            if prev != None: # check the prevsig
                self.assertEqual(unpacked[2], prev)

            prev = unpacked[-1] # set the prevsig

    #TODO v1 is deprecated
    """
    ...
    # TODO is deprecated? Sending registration for SIM supported?
    """
    @unittest.expectedFailure
